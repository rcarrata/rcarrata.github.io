...
But sometimes the folder is not accessible, because it's located on another host, or was deleted by mistake.

So... how can you regenerate your kubeconfig file and set client certificate authentication to system:admin?

NOTE: for obvious security reasons the kubeconfig file for system:admin can only be regenerated by a
cluster-admin user, so please log in with your cluster-admin privileged user to follow this procedure.
...
* In our case we want to request a CSR for the system:admin user and we also need to provide the CommonName as system:admin and the OrganizationName as masters:
...
Once we have the csr file generated from the earlier step, we need to create a signing request resource definition to request a certificate for our new kubeconfig file:
...
You can find more information about this procedure and about CSRs in the [AuthN-AuthZ Kubernetes section](https://kubernetes.io/docs/reference/access-authn-authz/certificate-signing-requests/#create-certificatesigningrequest)

### 4. Approve the CSR and extract the client certificate

* Now the Certificate Signing Request is waiting to be approved or denied. Let's approve it!
...
Now that we have our certificate on our localhost, let's use it. But how can we use our certificate within our new kubeconfig file?
...
Then, we need to create a context for the system:admin user.
...
* Based on the previous entry of the system:admin user, we can set up the context for our system:admin user:
...
* With the CA file we can add it to the new kubeconfig with the following command:
...
In this last step, we need to use the kubeconfig and test it against our OpenShift cluster.
...
I'm lazy sometimes to repeat the same process again and again, so I automated it in a simple bash script and [posted in a gist](https://gist.github.com/rcarrata/016da295c1421cccbfbd66ed9a7922bc)
