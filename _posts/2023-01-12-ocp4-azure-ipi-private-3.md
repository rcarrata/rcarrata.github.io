...
How can we generate Private clusters OpenShift in Azure, controlling our Outbound traffic with an scalable, managed Virtual NAT Gateway? What are the benefits of Azure Virtual NAT Gateways? And finally how we can configure our Azure Networking to use and benefit from the Virtual NAT?
...
Before deep dive into the OpenShift private clusters with Virtual NAT, let's explore what is a Virtual NAT Gateway in Azure. 
...
* **Security**: with a NAT gateway, our VMs and other resources that are part of our Private OpenShift clusters doesn't public IP addresses and can remain fully private.
* **Resiliency**: Because Virtual NAT is a fully managed and distributed system, and because always has multiple fault domains makes this service highly resilient.
...
* **Performance**: because it's based in an SDN service, NAT GW won't affect the network bandwidth of the computes resources.  
...
One of the reasons that we can ask is, why don't stick with the Azure Load Balancer for the Outbound traffic to the internet from our Azure vNets and Subnets?

Virtual Network NAT (NAT Gateway) is is the recommended method for outbound connectivity. NAT gateway doesn't have the same limitations of SNAT port exhaustion as does default outbound access and outbound rules of a load balancer.
...
Now that we have deploy the cluster, how we can access to the console that it's not exposed publicly?
