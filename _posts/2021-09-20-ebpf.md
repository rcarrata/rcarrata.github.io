...
What is eBPF and how can I use it in OpenShift 4? What are the tools and the benefits of eBPF programs? How can we run custom eBPF programs in our cluster to trace complex syscalls?
...
## 2. Kubectl Trace - A kubectl plugin based on eBPF programs

In many scenarios, eBPF is not used directly but indirectly via projects like bcc or bpftrace, which provide an abstraction on top of eBPF and do not require writing programs directly but instead offer the ability to specify intent-based definitions which are then implemented with eBPF.
...
There are a series of good examples in the [bpftrace repo](https://github.com/iovisor/bpftrace/tree/master/tools) that are handy, and some of them will be tested in this blog post.
...
Let's pick one node and run kubectl-trace with several eBPF bpftrace programs in our test cluster.

* Select one of the master nodes to run the kubectl trace eBPF programs on.
...
### 4.2 Tracing the Linux Virtual FileSystem calls in the kernel
...
This traces some common Linux Virtual FileSystem (VFS) calls and prints per-second summaries:

* Check the logs to see the trace calls of the VFS syscalls:
...
With that we demonstrated how to use eBPF programs in our OpenShift 4 clusters, and how we can benefit from the eBPF toolset and eBPF programs in OpenShift clusters.
