...
What is ARO4 and how can you install it in Azure? What are the procedures and the benefits of ARO4? How can we automate the installation and configuration procedure?
...
When we have the previous step ready, it's time to generate the infrastructure resource prerequisites in Azure for our ARO4 cluster:
...
* Create a resource group to allocate the resources for the ARO4 objects (like the VNets and subnets, and the ARO4 object itself):
...
NOTE: A valid pull-secret is needed for the installation. Please go to your [Cloud OpenShift Portal](cloud.redhat.com/openshift/) and get your pull-secret token.

* Then the az aro CLI will provision an ARO object in the Azure Portal:

[![](/images/aro4_1.png "aro 1")]({{site.url}}/images/aro4_1.png)

* Automatically, an additional Azure Resource Group is created with the Azure objects for the installation, managed by the ARO SREs from RH and MSFT:

[![](/images/aro4_2.png "aro 2")]({{site.url}}/images/aro4_2.png)

* Inside this Resource Group we can see the resources generated that are needed to provision and configure the ARO4 cluster:
...
* To get access to the cluster, list credentials for your cluster:
...
If you want to install all the Azure infrastructure resource prerequisites and the ARO4 cluster in a simple way, we generated a script to automate the whole process described before.
...
* Tweak the parameters inside the script to adjust to your region, cluster_name, and network CIDRs, among others:
