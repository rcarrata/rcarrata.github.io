...
How we can analyze our Network Flows in our Kubernetes clusters? How we can enable Network Observability for Kubernetes in a simple, searchable and visual way? How we can leverage cool technologies such as eBPF or IPFIX to enable the Network Observability for our K8s Network Traffic?

Let's dig in!

This is the third blog post of the Network Observability in Kubernetes / OpenShift. 
...
NetObserv Operator is is a Kubernetes / OpenShift operator for network observability. This operator deploys a monitoring pipeline collecting and enriching Network traffic Flows.
...
Let's install it and start to play with this operator, to see how can help us in our Network Observability for our Kubernetes / OpenShift clusters.
...
- **Sampling**: value of flows sampled. By default is set to 50 (1:50) for EBPF and 400 for IPFIX (1:400). This means that one flow of every 50 is sampled.
...
When we installed the Grafana dashboard and a Grafana Data Source, we deployed a Grafana Dashboard already ready to to retrieve and show the collected Network Flow from Loki.
...
Because we love GitOps in this blog, lets deploy all of this steps (including Prerequisites) using GitOps and ArgoCD!
